...
void EvilPortal::setup() {
  ...
  html_files->add("Back");
    ...
    if (sd_obj.supported) {
      ...
      SERIAL_AT.println("Evil Portal Found " + (String)html_files->size() + " HTML files");
    }
  #endif
}
...
void EvilPortal::setupServer() {
  server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
    ...
    SERIAL_AT.println("client connected");
    #ifdef HAS_SCREEN
  ...
  SERIAL_AT.println("web server up");
}

void EvilPortal::setHtmlFromSerial() {
  SERIAL_AT.println("Setting HTML from serial...");
  const char *htmlStr = SERIAL_AT.readString().c_str();
  strncpy(index_html, htmlStr, strlen(htmlStr));
  this->has_html = true;
  this->using_serial_html = true;
  SERIAL_AT.println("html set");
}

bool EvilPortal::setHtml() {
  if (this->using_serial_html) {
    SERIAL_AT.println("html previously set");
    return true;
  }
  SERIAL_AT.println("Setting HTML...");
  #ifdef HAS_SD
  ...
  if (!html_file) {
    ...
    #endif
    SERIAL_AT.println("Could not find /" + this->target_html_name + ". Use stopscan...");
    return false;
  }
  else {
    if (html_file.size() > MAX_HTML_SIZE) {
        ...
        this->sendToDisplay("Touch to exit...");
      #endif
      SERIAL_AT.println("The provided HTML is too large. Byte limit is " + (String)MAX_HTML_SIZE + "\nUse stopscan...");
      return false;
    }
    String html = "";
...
bool EvilPortal::setAP(LinkedList<ssid>* ssids, LinkedList<AccessPoint>* access_points) {
  ...
  if ((ssids->size() <= 0) && (temp_ap_name == "")) {
    ...
    if (!ap_config_file) {
        ...
        this->sendToDisplay("Touch to exit...");
      #endif
      SERIAL_AT.println("Could not find /ap.config.txt. Use stopscan...");
      return false;
    }
    // Config file good. Proceed
      ...
      if (ap_config_file.size() > MAX_AP_NAME_SIZE) {
          ...
          this->sendToDisplay("Touch to exit...");
        #endif
        SERIAL_AT.println("The provided AP name is too large. Byte limit is " + (String)MAX_AP_NAME_SIZE + "\nUse stopscan...");
        return false;
      }
      ...
      #endif
      SERIAL_AT.println("AP name from config file: " + ap_config);
  ...
  // Priority is SSID list before AP selected and config file
    ...
    if (ap_config.length() > MAX_AP_NAME_SIZE) {
      ...
      this->sendToDisplay("AP name: " + ap_config);
    #endif
    SERIAL_AT.println("AP name from SSID list: " + ap_config);
  }
    ...
    if (temp_ap_name.length() > MAX_AP_NAME_SIZE) {
        ...
        this->sendToDisplay("Touch to exit...");
      #endif
      SERIAL_AT.println("The given AP name is too large. Byte limit is " + (String)MAX_AP_NAME_SIZE + "\nUse stopscan...");
    }
    else {
      ap_config = temp_ap_name;
        ...
        this->sendToDisplay("AP name: " + ap_config);
      #endif
      SERIAL_AT.println("AP name from AP list: " + ap_config);
    }
  }
  else {
    SERIAL_AT.println("Could not configure Access Point. Use stopscan...");
    #ifdef HAS_SCREEN
  ...
  if (ap_config != "") {
    ...
    this->has_ap = true;
    SERIAL_AT.println("ap config set");
    return true;
  }
  else
    return false;
...
void EvilPortal::startAP() {
  ...
  SERIAL_AT.print("starting ap ");
  SERIAL_AT.println(apName);
  ...
  SERIAL_AT.print("ap ip address: ");
  SERIAL_AT.println(WiFi.softAPIP());
