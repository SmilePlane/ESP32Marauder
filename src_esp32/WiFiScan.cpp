...
extern "C" {
  ...
  NimBLEAdvertisementData WiFiScan::GetUniversalAdvertisementData(EBLEPayloadType Type) {
    ...
    switch (Type) {
      ...
      default: {
        SERIAL_AT.println("Please Provide a Company Type");
        break;
      }
    }
      ...
      void onResult(BLEAdvertisedDevice *advertisedDevice) {
        ...
        if (wifi_scan_obj.currentScanMode == BT_SCAN_ALL) {
          if (buf >= 0)
          {
            ...
            SERIAL_AT.print(" RSSI: ");
            SERIAL_AT.print(advertisedDevice->getRSSI());
    
            display_string.concat(" ");
            SERIAL_AT.print(" ");
            
            SERIAL_AT.print("Device: ");
            if(advertisedDevice->getName().length() != 0)
            {
              display_string.concat(advertisedDevice->getName().c_str());
              SERIAL_AT.print(advertisedDevice->getName().c_str());
              
            }
            else
            {
              display_string.concat(advertisedDevice->getAddress().toString().c_str());
              SERIAL_AT.print(advertisedDevice->getAddress().toString().c_str());
            }
  ...
  class bluetoothScanSkimmersCallback: public BLEAdvertisedDeviceCallbacks {
      void onResult(BLEAdvertisedDevice *advertisedDevice) {
        ...
        if (buf >= 0)
        {
          ...
          SERIAL_AT.print(" RSSI: ");
          SERIAL_AT.println(advertisedDevice->getRSSI());
        }
      }
  };
#endif
...
int WiFiScan::clearStations() {
  ...
  stations->clear();
  SERIAL_AT.println("stations: " + (String)stations->size());
...
int WiFiScan::clearAPs() {
  ...
  SERIAL_AT.println("access_points: " + (String)access_points->size());
  return num_cleared;
}
...
int WiFiScan::clearSSIDs() {
  ...
  ssids->clear();
  SERIAL_AT.println("ssids: " + (String)ssids->size());
  return num_cleared;
}
...
bool WiFiScan::addSSID(String essid) {
  ...
  ssids->add(s);
  SERIAL_AT.println(ssids->get(ssids->size() - 1).essid);
...
/*void WiFiScan::joinWiFi(String ssid, String password)
{
  ...
  if ((WiFi.status() == WL_CONNECTED) && (ssid == connected_network) && (ssid != "")) {
    ...
    SERIAL_AT.println("Already connected. Disconnecting...");
    WiFi.disconnect();
  }
  ...
  SERIAL_AT.print("Connecting to WiFi");
  ...
  SERIAL_AT.print("IP address: ");
  SERIAL_AT.println(WiFi.localIP());
  this->wifi_initialized = true;
}*/
...
void WiFiScan::initWiFi(uint8_t scan_mode) {
  ...
  if (scan_mode != WIFI_SCAN_OFF) {
    //SERIAL_AT.println(F("Initializing WiFi settings..."));
    this->changeChannel();
    ...
    //SERIAL_AT.println(F("Initialization complete"));
  }
}
...
void WiFiScan::StopScan(uint8_t scan_mode)
{
  ...
  (currentScanMode == BT_SCAN_SKIMMERS))
  {
    ...
    #endif
    //SERIAL_AT.print("display_buffer->size(): ");
    SERIAL_AT.println(display_obj.display_buffer->size());
...
void WiFiScan::RunLoadAPList() {
    ...
    if (!file) {
      SERIAL_AT.println("Could not open /APs_0.log");
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
    ...
    DeSERIAL_ATizationError error = deSERIAL_ATizeJson(doc, file);
    if (error) {
      SERIAL_AT.print("JSON deSERIAL_ATize error: ");
      SERIAL_AT.println(error.c_str());
      file.close();
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setFreeFont(NULL);
        ...
        display_obj.tft.println("Could not deSERIAL_ATize JSON");
    ...
    #endif
    SERIAL_AT.print("Loaded APs:");
    SERIAL_AT.println((String)access_points->size());
  #endif
}
...
void WiFiScan::RunLoadSSIDList() {
    ...
    if (!log_file) {
      SERIAL_AT.println("Could not open /SSIDs_0.log");
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setFreeFont(NULL);
        display_obj.tft.setCursor(0, 100);
    ...
    SERIAL_AT.print("Loaded SSIDs: ");
    SERIAL_AT.println((String)ssids->size());
  #endif
}
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color)
{
  ...
  //  SERIAL_AT.println("Could not successfully start EvilPortal. Setting WIFI_SCAN_OFF...");
  //  this->StartScan(WIFI_SCAN_OFF, TFT_MAGENTA);
  //  return;
  //}
  //else
  //  SERIAL_AT.println("Setup EvilPortal. Current mode: " + this->currentScanMode);
  this->wifi_initialized = true;
  initTime = millis();
}
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color)
{
  ...
  SERIAL_AT.println(text_table4[9] + (String)access_points->size());
  #ifdef HAS_SCREEN
    display_obj.TOP_FIXED_AREA_2 = 48;
    display_obj.tteBar = true;
    display_obj.print_delay_1 = 15;
  ...
  void WiFiScan::RunLvJoinWiFi(uint8_t scan_mode, uint16_t color) {
    ...
    display_obj.tft.setRotation(1);
      ...
      SERIAL_AT.println("Using TFT Shield");
    #else if defined(TFT_DIY)
      uint16_t calData[5] = { 213, 3469, 320, 3446, 1 }; // Landscape TFT DIY
      SERIAL_AT.println("Using TFT DIY");
    #endif
    #ifdef HAS_ILI9341
...
void WiFiScan::RunGPSInfo() {
    ...
    SERIAL_AT.println("Refreshing GPS Data on screen...");
    #ifdef HAS_SCREEN
    ...
    // Display to SERIAL_AT
    SERIAL_AT.println("==== GPS Data ====");
    if (gps_obj.getFixStatus())
      SERIAL_AT.println("  Good Fix: Yes");
    else
      SERIAL_AT.println("  Good Fix: No");
      
    if(text != "") SERIAL_AT.println("      Text: " + text);

    SERIAL_AT.println("Satellites: " + gps_obj.getNumSatsString());
    SERIAL_AT.println("  Accuracy: " + (String)gps_obj.getAccuracy());
    SERIAL_AT.println("  Latitude: " + gps_obj.getLat());
    SERIAL_AT.println(" Longitude: " + gps_obj.getLon());
    SERIAL_AT.println("  Altitude: " + (String)gps_obj.getAlt());
    SERIAL_AT.println("  Datetime: " + gps_obj.getDatetime());
  #endif
}
...
void WiFiScan::RunGPSNmea() {
    ...
    if(buffer && queue_enabled){
      ...
      if(size){
        ...
        for(int i=0;i<size;i++){
          ...
          SERIAL_AT.println(line.sentence);
      ...
      if(nmea_sentence != "" && nmea_sentence != old_nmea_sentence){
        ...
        SERIAL_AT.println(nmea_sentence);
      }
    ...
    gps_obj.sendSentence(SERIAL_AT, gxgga.c_str());
    gps_obj.sendSentence(SERIAL_AT, gxrmc.c_str());
...
void WiFiScan::RunPacketMonitor(uint8_t scan_mode, uint16_t color)
{
  ...
  SERIAL_AT.println("Running packet scan...");
  esp_wifi_init(&cfg);
  esp_wifi_set_storage(WIFI_STORAGE_RAM);
  esp_wifi_set_mode(WIFI_MODE_NULL);
  esp_wifi_start();
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color)
{
  ...
  if (err != 0)
  {
    SERIAL_AT.print("could not set protocol : err=0x");
    SERIAL_AT.println(err, HEX);
  }
  ...
  if (err != 0)
  {
    SERIAL_AT.print("AP config set error, Maurauder SSID might visible : err=0x");
    SERIAL_AT.println(err, HEX);
  }
...
void WiFiScan::executeWarDrive() {
    ...
    if (gps_obj.getGpsModuleStatus()) {
      ...
      while (WiFi.scanComplete() == WIFI_SCAN_RUNNING) {
        SERIAL_AT.println("Scan running...");
        delay(500);
      }
      ...
      if (n > 0) {
        for (int i = 0; i < n; i++) {
          ...
          SERIAL_AT.print((String)this->mac_history_cursor + " | " + wardrive_line);
...
void WiFiScan::RunBluetoothScan(uint8_t scan_mode, uint16_t color)
{
    ...
    SERIAL_AT.println("Started BLE Scan");
    this->ble_initialized = true;
    initTime = millis();
  #endif
}
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      if (src == "de:ad:be:ef:de:ad") {
        ...
        delay(random(0, 10));
        SERIAL_AT.print("RSSI: ");
        SERIAL_AT.print(snifferPacket->rx_ctrl.rssi);
        SERIAL_AT.print(" Ch: ");
        SERIAL_AT.print(snifferPacket->rx_ctrl.channel);
        SERIAL_AT.print(" BSSID: ");
        SERIAL_AT.print(addr);
        //display_string.concat(addr);
        display_string.concat("CH: " + (String)snifferPacket->rx_ctrl.channel);
        SERIAL_AT.print(" ESSID: ");
        display_string.concat(" -> ");
        ...
        {
          SERIAL_AT.print((char)snifferPacket->payload[i + 38]);
          //display_string.concat((char)snifferPacket->payload[i + 38]);
          if (isAscii(snifferPacket->payload[i + 38]))
            essid.concat((char)snifferPacket->payload[i + 38]);
          else
            SERIAL_AT.println("Got non-ascii character: " + (String)(char)snifferPacket->payload[i + 38]);
        }
        //essid.concat("\": \"\"}}");
...
void WiFiScan::apSnifferCallbackFull(void* buf, wifi_promiscuous_pkt_type_t type) {  
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        mac_match = true;
        //SERIAL_AT.print("Checking ");
        //SERIAL_AT.print(addr);
        //SERIAL_AT.println(" against " + (String)access_points->get(i).essid);

        
        for (int x = 0; x < 6; x++) {
          //SERIAL_AT.println((String)snifferPacket->payload[x + 10] + " | " + (String)access_points->get(i).bssid[x]);
          if (snifferPacket->payload[x + 10] != access_points->get(i).bssid[x]) {
            mac_match = false;
            //SERIAL_AT.println("MACs do not match");
            break;
          }
        }
        if (mac_match) {
      ...
      if (!in_list) {
        ...
        SERIAL_AT.print(access_points->size());
        SERIAL_AT.print(" ");
        SERIAL_AT.print(esp_get_free_heap_size());
      ...
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
        ...
        for (int i = 0; i < access_points->size(); i++) {
          if (access_points->get(i).selected) {
            ...
            if (found) {
              //SERIAL_AT.println("Received beacon from " + access_points->get(i).essid + ". Checking RSSI...");
        ...
        if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) || (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
          ...
          SERIAL_AT.println((String)access_points->get(targ_index).essid + " RSSI: " + (String)access_points->get(targ_index).rssi);
          return;
        }
      }
        ...
        buffer_obj.append(snifferPacket, len);
          ...
          if (gps_obj.getGpsModuleStatus()) {
            ...
            if (do_save) {
              if (n > 0) {
                for (int i = 0; i < n; i++) {
                  SERIAL_AT.printf("%-32.32s", WiFi.SSID(i).c_str());
                  SERIAL_AT.print(" -> ");
                  SERIAL_AT.println(wifi_scan_obj.security_int_to_string(WiFi.encryptionType(i)).c_str());
                }
              }
              ...
              SERIAL_AT.println(wardrive_line);
              //buffer_obj.append(wardrive_line);
            }
          }
        #endif
...
void WiFiScan::stationSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  // Print findings to SERIAL_AT
  SERIAL_AT.print((String)stations->size() + ": ");
  
  char sta_addr[] = "00:00:00:00:00:00";
  
  if (ap_is_src) {
    SERIAL_AT.print("ap: ");
    SERIAL_AT.print(ap_addr);
    SERIAL_AT.print(" -> sta: ");
    getMAC(sta_addr, snifferPacket->payload, 4);
    SERIAL_AT.println(sta_addr);
  }
  else {
    SERIAL_AT.print("sta: ");
    getMAC(sta_addr, snifferPacket->payload, 10);
    SERIAL_AT.print(sta_addr);
    SERIAL_AT.print(" -> ap: ");
    SERIAL_AT.println(ap_addr);
  }
...
void WiFiScan::rawSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
    ...
    if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) || (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
      targ_ap.rssi = snifferPacket->rx_ctrl.rssi;
      access_points->set(targ_index, targ_ap);
      SERIAL_AT.print((String)access_points->get(targ_index).essid + " RSSI: " + (String)access_points->get(targ_index).rssi);
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x40) && (buf == 0))
    {
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_PROBE) {
        ...
        SERIAL_AT.print(" Ch: ");
        SERIAL_AT.print(snifferPacket->rx_ctrl.channel);
        SERIAL_AT.print(" Client: ");
        char addr[] = "00:00:00:00:00:00";
        getMAC(addr, snifferPacket->payload, 10);
        SERIAL_AT.print(addr);
        display_string.concat(addr);
        SERIAL_AT.print(" Requesting: ");
        display_string.concat(" -> ");
        for (int i = 0; i < snifferPacket->payload[25]; i++)
        {
          SERIAL_AT.print((char)snifferPacket->payload[26 + i]);
    ...
    if ((snifferPacket->payload[0] == 0x40) && (buf == 0))
    {
      ...
      for (int i = 0; i < ssids->size(); i++) {
        if (ssids->get(i).essid == essid) {
          SERIAL_AT.println("Found a sheep");
          found = true;
          break;
        }
      }
    ...
    int temp_len = display_string.length();
      ...
      //SERIAL_AT.print(" ");
        ...
        if (display_obj.display_buffer->size() == 0)
        {
          ...
          display_obj.display_buffer->add(display_string);
          display_obj.loading = false;
          SERIAL_AT.println(display_string);
        }
      #endif
    #endif
...
void WiFiScan::eapolSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (( (snifferPacket->payload[30] == 0x88 && snifferPacket->payload[31] == 0x8e)|| ( snifferPacket->payload[32] == 0x88 && snifferPacket->payload[33] == 0x8e) )){
    num_eapol++;
    SERIAL_AT.println("Received EAPOL:");
...
void WiFiScan::activeEapolSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
    ...
    if (wifi_scan_obj.currentScanMode == WIFI_SCAN_ACTIVE_LIST_EAPOL) {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        if (access_points->get(i).selected) {
          ...
          if (found) {
            SERIAL_AT.println("Received beacon from " + access_points->get(i).essid + ". Deauthenticating...");
            break;
          }
        }
      }
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      if (pressed) {
        SERIAL_AT.print("Got touch | X: ");
        SERIAL_AT.print(t_x);
        SERIAL_AT.print(" Y: ");
        SERIAL_AT.println(t_y);
      }
      ...
      for (uint8_t b = 0; b < BUTTON_ARRAY_LEN; b++)
      {
        ...
        {
          SERIAL_AT.println("Bro, key pressed");
          //do_break = true;
        }
        ...
        if (display_obj.key[b].justReleased())
        {
          ...
          if (b == 4) {
            if (set_channel > 1) {
              SERIAL_AT.println("Shit channel down");
              set_channel--;
              delay(70);
          ...
          // Channel + button pressed
            ...
            if (set_channel < MAX_CHANNEL) {
              SERIAL_AT.println("Shit channel up");
              set_channel++;
              delay(70);
              display_obj.tft.fillRect(127, 0, 193, 28, TFT_BLACK);
              display_obj.tftDrawChannelScaleButtons(set_channel);
            ...
            SERIAL_AT.println("Exiting packet monitor...");
      ...
      if (currentTime - initTime >= (GRAPH_REFRESH * 5)) {
        ...
        if (y_pos_x >= HEIGHT_1) {
          SERIAL_AT.println("Max EAPOL number reached. Adjusting...");
          num_eapol = 0;
        }
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      if (currentTime - initTime >= GRAPH_REFRESH) {
        //SERIAL_AT.println("-----------------------------------------");
        //SERIAL_AT.println("Time elapsed: " + (String)(currentTime - initTime) + "ms");
        x_pos += x_scale;
        initTime = millis();
        ...
        //SERIAL_AT.println("num_beacon: " + (String)num_beacon);
        //SERIAL_AT.println("num_deauth: " + (String)num_deauth);
        //SERIAL_AT.println(" num_probe: " + (String)num_probe);
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL)
  {
    ...
    {
      initTime = millis();
      //SERIAL_AT.print("packets/sec: ");
      //SERIAL_AT.println(packets_sent);
      String displayString = "";
      String displayString2 = "";
      displayString.concat(text18);
      displayString.concat(packets_sent);
