...
bool Settings::begin() {
  if(!SPIFFS.begin(FORMAT_SPIFFS_IF_FAILED)){
    SERIAL_AT.println("Settings SPIFFS Mount Failed");
    return false;
  }
  ...
  if (SPIFFS.exists("/settings.json")) {
    ...
    if (!settingsFile) {
      settingsFile.close();
      SERIAL_AT.println(F("Could not find settings file"));
    ...
    SERIAL_AT.println("Settings file does not exist");
    if (this->createDefaultSettings(SPIFFS))
      return true;
    else
      return false;
  ...
  //SERIAL_AT.println("Settings: " + (String)json_string + "\n");
...
bool Settings::saveSetting<bool>(String key, bool value) {
  ...
  for (int i = 0; i < json["Settings"].size(); i++) {
    if (json["Settings"][i]["name"].as<String>() == key) {
      ...
      SERIAL_AT.println("Saving setting...");

      File settingsFile = SPIFFS.open("/settings.json", FILE_WRITE);

      if (!settingsFile) {
        SERIAL_AT.println(F("Failed to create settings file"));
        return false;
      }

      if (serializeJson(json, settingsFile) == 0) {
        SERIAL_AT.println(F("Failed to write to file"));
      }
      if (serializeJson(json, settings_string) == 0) {
        SERIAL_AT.println(F("Failed to write to string"));
      }
...
bool Settings::toggleSetting(String key) {
  JsonDocument json; // ArduinoJson v6
  ...
  for (int i = 0; i < json["Settings"].size(); i++) {
    if (json["Settings"][i]["name"].as<String>() == key) {
      if (json["Settings"][i]["value"]) {
        ...
        SERIAL_AT.println("Setting value to false");
        return false;
      }
      else {
        saveSetting<bool>(key, true);
        SERIAL_AT.println("Setting value to true");
        return true;
      }
...
void Settings::printJsonSettings(String json_string) {
  ...
  SERIAL_AT.println("Settings\n----------------------------------------------");
  for (int i = 0; i < json["Settings"].size(); i++) {
    SERIAL_AT.println("Name: " + json["Settings"][i]["name"].as<String>());
    SERIAL_AT.println("Type: " + json["Settings"][i]["type"].as<String>());
    SERIAL_AT.println("Value: " + json["Settings"][i]["value"].as<String>());
    SERIAL_AT.println("----------------------------------------------");
  }
}

bool Settings::createDefaultSettings(fs::FS &fs) {
  SERIAL_AT.println(F("Creating default settings file: settings.json"));
